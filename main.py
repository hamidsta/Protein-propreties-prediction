...
def find_best_model_AUC(data,label,x_train, y_train, tuned_params):
    ...
    print('*** Here is the statistics XGB AGAINST LR AUC score :',statistical_result['lr_vs_xgb'])
...
def main():
    ...
    while True :
        try :
            file_dir = str(input( ' Put the path of the CSV file who contain the value to predict. Must be on the same format as the one use for the technical  test '))
            ...
            x_train, x_test, y_train, y_test = train_test_split(data, label, test_size=0.3, random_state=42,
                                                                stratify=label)  # train/test
            print('file have been splited into test/training ')
            ...
            user_quit=str(input('** Do you want to test another file ? Type yes/y if you want to test AGAIN   '))
    ...
    while True:
        xt = input("""
            ...
            CHOOSE Function >>>>""")
        if xt =='1':
            print('*** run with  linear model')
            lr_fit = fit_and_run(x_train, y_train, y_test,x_test,final_thune,'linear_model')

        if xt =='2':
            print('*** run with  RandomForestClassifier')
            rf_fit=fit_and_run(x_train, y_train, y_test,x_test,final_thune,'RandomForestClassifier')

        if xt == '3':
            print('*** run with  XGBClassifier')
            xgb_fit=fit_and_run(x_train, y_train, y_test,x_test,final_thune,'XGBClassifier')

        if xt =='4':
            print('*** test statistics')
            find_best_model_AUC(data,label, x_train, y_train, final_thune)

        if xt =='5':
            plot_dir=str(input('*** Put existing path were plot will be stored '))
            if os.path.isdir(plot_dir) == True :
                print('*** Plot Validation training size AUC score , last argument is directory')
                plot_learning_curves(x_train, y_train, final_thune,plot_dir)
                print('*** Plot mean roc curves')
                plot_roc_curves(x_train, y_train, final_thune,plot_dir)
            else:
                print('*** this is not a directory')
                pass


        # predict the property P1f for testing set
        ...
        # Ability for the user to predict the property P1 for any given smile

        if xt =='6' :
            try :
                given_smile = str(input('Put CSV file whith smile molecule to predict, *** PUT CSV FILE WITH THE SAME FORMAT AS THE ONE USE FOR TECHNICAL TEST *** :  '))
                if given_smile.endswith(".csv"):
                    df = pd.read_csv(given_smile, sep=',')
                    df['Morgan_fingerprint'] = df['smiles'].map(computeMorganFP)  # get morganFingerprint as binary array
                    ...
                    if data.shape[1] != 2048:
                        ...
                        print('*** run with  Linear regression')
                        lr_fit = fit_and_run(x_train, y_train, y_test, x_test, final_thune, 'linear_model')
                        lr_predict = (lr_fit.predict(data))  # Linear
                        print('** For Linear regression here is the prediction :' ,lr_predict)
                else:
                    print('file need to be CSV ')
            except FileNotFoundError:
                print('*** directory name not found ! ')

        if xt=='7':
            try:
                given_smile = str(input('Put CSV file whith smile molecule to predict  '))
                if given_smile.endswith(".csv"):
                    df = pd.read_csv(given_smile, sep=',')
                    df['Morgan_fingerprint'] = df['smiles'].map(
                        computeMorganFP)  # get morganFingerprint as binary array
                    data = df.Morgan_fingerprint.apply(pd.Series)
                    if data.shape[1] != 2048:
                        print('csv of binary feature NEED to be of  length 2048, otherwise dont work ')
                        pass
                        ...
                        print('** For RANDOMFOREST here is the prediction :' ,rf_predict)
                else:
                    print('file need to be CSV ')
            except FileNotFoundError:
                print('*** directory name not found ! ')
        ...
        if xt =='8':
            try:
                ...
                if given_smile.endswith(".csv"):
                    ...
                    data = df.Morgan_fingerprint.apply(pd.Series)
                    if data.shape[1] != 2048:
                        print('csv of binary feature NEED to be of  length 2048, otherwise dont work ') # just to be sure , but cannot be different than 2048 because i already set it
                    else:
                        print('*** run with XGBOOST')
                        xgb_fit = fit_and_run(x_train, y_train, y_test, x_test, final_thune, 'XGBClassifier')
                        xgb_predict = (xgb_fit.predict(data))  # XGBOOST
                        print('** For XGBOOST here is the prediction :', xgb_predict)
                else:
                    print('file need to be CSV ')
            except FileNotFoundError:
                print('*** directory name not found ! ')
